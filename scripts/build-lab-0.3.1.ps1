...
$StagingDist = Join-Path $ProjectRoot "build\release-pc-lab-0.3.1"
$StagingWork = Join-Path $ProjectRoot "build\motionbridge-lab-0.3.1"
$StagingExe = Join-Path $StagingDist "MotionBridge-0.3.1-lab-mvp.exe"
$ExePath = Join-Path $ProjectRoot "dist\MotionBridge-0.3.1-lab-mvp.exe"
...
try {
    & $VenvPython -m PyInstaller --noconfirm --clean --distpath $StagingDist --workpath $StagingWork motionbridge-lab-0.3.1.spec
    if ($LASTEXITCODE -ne 0) {
