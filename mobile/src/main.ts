...
app.innerHTML = `
  ...
  <header><div><p>MOTIONBRIDGE 0.7.7</p><h1 id="pageTitle">体感控制</h1></div><div id="connectionBadge" class="badge"><i></i><b>未连接电脑</b></div></header>
  <main>
    ...
    <section class="runtime-card hidden" id="runtimeCard">
      <div class="status-row"><button id="statusToggle" class="status-summary" type="button" aria-expanded="false"><strong id="sendState">等待完整人体</strong><span>摄像头 <b id="cameraFps">0 FPS</b></span><span>识别 <b id="localFps">0 FPS</b></span><span>Full33</span><span id="delegateStatus">—</span><small id="modelError"></small></button><button id="hideStatus" class="status-hide" type="button">隐藏</button></div>
      <div class="status-drawer hidden" id="statusDrawer">
        <label class="runtime-select">镜头<select id="runtimeCameraDevice"><option value="__auto__">自动</option></select></label>
        <button id="runtimeFlip" class="text-button">切换镜头</button>
        <div class="drawer-voice" id="voiceControl"><label><input id="voiceToggle" type="checkbox">语音控制</label><span id="voiceState">关闭</span></div>
        <button id="stopButton" class="stop">停止识别</button>
      </div>
    </section>
    <button id="showStatus" class="status-show hidden" type="button">展开</button>
...
const statusDrawer = document.querySelector<HTMLElement>("#statusDrawer")!;
const statusToggle = document.querySelector<HTMLButtonElement>("#statusToggle")!;
...
async function stop(): Promise<void> { running = false; cameraFrameLoop = false; if (reconnectTimer != null) window.clearTimeout(reconnectTimer); socket?.close(); socket = null; poseLandmarker?.close(); poseLandmarker = null; clearWebCamera(); motionDebug.videoReady = false; motionDebug.videoWidth = 0; motionDebug.videoHeight = 0; statusDrawer.classList.add("hidden"); statusToggle.setAttribute("aria-expanded", "false"); await stopVoiceControl(); await wakeLock?.release().catch(() => {}); wakeLock = null; context.clearRect(0, 0, canvas.width, canvas.height); setupCard.classList.remove("hidden"); runtimeCard.classList.add("hidden"); showStatus.classList.add("hidden"); document.querySelector("#guide")!.classList.add("hidden"); setConnection("offline"); }
async function chooseCamera(cameraId: string): Promise<void> { selectedCameraDeviceId = cameraId; cameraDeviceSelect.value = cameraId; runtimeCameraDevice.value = cameraId; if (running) await startCamera(); applyMirror(); }
async function flipCamera(): Promise<void> { facingMode = facingMode === "environment" ? "user" : "environment"; selectedCameraDeviceId = "__auto__"; cameraDeviceSelect.value = selectedCameraDeviceId; runtimeCameraDevice.value = selectedCameraDeviceId; if (running) await startCamera(); applyMirror(); }
...
statusToggle.addEventListener("click", () => { const open = statusDrawer.classList.toggle("hidden"); statusToggle.setAttribute("aria-expanded", String(!open)); });
hideStatus.addEventListener("click", () => { statusDrawer.classList.add("hidden"); statusToggle.setAttribute("aria-expanded", "false"); runtimeCard.classList.add("hidden"); showStatus.classList.remove("hidden"); });
