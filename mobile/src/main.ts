...
app.innerHTML = `
  ...
  <main>
    ...
    <section class="runtime-card hidden" id="runtimeCard">
      ...
      <div class="runtime-metrics"><span>摄像头帧率 <b id="cameraFps">0 FPS</b></span><span>识别帧率 <b id="localFps">0 FPS</b></span><span>模型 <b>MediaPipe Pose Full · 33点</b></span></div>
...
async function createPose(): Promise<PoseLandmarker> { const files = await getVision(); return await PoseLandmarker.createFromOptions(files, { baseOptions: { modelAssetPath: motionDebug.modelUrl, delegate: "CPU" }, runningMode: "VIDEO", numPoses: 2, minPoseDetectionConfidence: .55, minPosePresenceConfidence: .55, minTrackingConfidence: .55 }); }
async function loadPoseModel(): Promise<void> { document.querySelector("#loading")!.classList.remove("hidden"); motionDebug.modelState = "loading"; motionDebug.detectCalls = 0; motionDebug.lastPoseCount = 0; motionDebug.lastInferenceMs = 0; motionDebug.lastDetectError = null; try { poseLandmarker?.close(); poseLandmarker = await createPose(); motionDebug.modelState = "ready"; document.querySelector("#loading")!.classList.add("hidden"); } catch (error) { motionDebug.modelState = "error"; motionDebug.lastDetectError = error instanceof Error ? error.message : String(error); document.querySelector("#loadingText")!.textContent = `模型错误：${motionDebug.lastDetectError}`; document.querySelector("#loading")!.classList.add("hidden"); throw error; } }
