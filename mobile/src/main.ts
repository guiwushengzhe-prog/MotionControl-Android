...
app.innerHTML = `
  ...
  <header><div><p>MOTIONBRIDGE 0.7.6</p><h1 id="pageTitle">体感控制</h1></div><div id="connectionBadge" class="badge"><i></i><b>未连接电脑</b></div></header>
  <main>
    ...
    <section class="runtime-card hidden" id="runtimeCard">
      <button id="statusToggle" class="status-summary" type="button" aria-expanded="false"><strong id="sendState">等待完整人体</strong><span>摄像头 <b id="cameraFps">0 FPS</b></span><span>识别 <b id="localFps">0 FPS</b></span><span>Full33</span><span id="delegateStatus">—</span><small id="modelError"></small></button>
...
document.querySelector("#statusToggle")!.addEventListener("click", () => { const open = statusDrawer.classList.toggle("hidden"); document.querySelector("#statusToggle")?.setAttribute("aria-expanded", String(!open)); });
